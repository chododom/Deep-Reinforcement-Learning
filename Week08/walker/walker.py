...
parser.add_argument("--batch_size", default=69, type=int, help="Batch size.")
    ...
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        class Actor(tf.keras.Model):
            def __init__(self):
                ...
                self._log_alpha = tf.Variable(np.log(1), dtype=tf.float32)
    ...
    while training:
        for _ in range(args.evaluate_each):
            ...
            if len(replay_buffer) >= 4 * args.batch_size:
                ...
                #print(rewards.shape)
                #print(network.predict_values(next_states).shape)
                ...
                returns =  rewards + args.gamma * (~dones) * np.reshape(network.predict_values(next_states), (-1, args.batch_size))
