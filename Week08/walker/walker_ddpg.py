...
class Network:
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        #
        # - two critics, starting with given states and actions, producing predicted
        ...
        #
        # - two target critics as the copy of the critics using `tf.keras.models.clone_model`.
        ...
        actor = tf.keras.layers.Dense(256, activation='relu')(actor)
        ...
        # Critic network
        states_critic = tf.keras.layers.Dense(512, activation='relu')(input_states)
        ...
        states_critic2 = tf.keras.layers.Dense(512, activation='relu')(input_states)
        actions_critic2 = tf.keras.layers.Input(env.action_space.shape)
        critic2 = tf.keras.layers.concatenate([states_critic2, actions_critic2])
        ...
        self.critic2 = tf.keras.Model(inputs=[input_states, actions_critic2], outputs=critic2)
    ...
    def predict_values(self, states: np.ndarray) -> np.ndarray:
        ...
        critic = self.target_critic([states, actor])
        critic2 = self.target_critic2([states, actor])
...
def main(env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
    ...
    if args.recodex:        
        network.actor = tf.keras.models.load_model('actor280.h5')
        network.target_actor = tf.keras.models.load_model('actor280.h5')
        network.critic = tf.keras.models.load_model('critic280.h5')
        network.target_critic = tf.keras.models.load_model('critic280.h5')
        ...
        while training:
            ...
            for _ in range(args.evaluate_each):
                ...
                while not done:
                    ...
                    action = np.clip(action, env.action_space.low, env.action_space.high) # env.action_space.low/high
    
                    next_state, reward, terminated, truncated, _ = env.step(action)
                    done = terminated or truncated
                    
                    reward = -15 if reward == -100 else reward
            ...
            if mean_returns >= best_returns:
                ...
                tactor_name = f"tactor{int(best_returns)}.h5"
                tcritic_name = f"tcritic{int(best_returns)}.h5"
