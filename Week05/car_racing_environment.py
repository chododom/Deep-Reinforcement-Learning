...
class CarRacingFS(gym.Env, EzPickle):
        ...
        for _ in range(self.frame_skip):
            ...
            if action is not None:  # First step without action, called from reset()
                ...
                if abs(x) > PLAYFIELD or abs(y) > PLAYFIELD:
                    terminated = True
                    self.reward = self.prev_reward - 100
    ...
    def _render_road(self, zoom, translation, angle, scroll):
        ...
        for x in range(-20, 20, 2):
            if (x * PLAYFIELD / 20 - scroll[0]) ** 2 >= mindist: continue
            for y in range(-20, 20, 2):
                if (y * PLAYFIELD / 20 - scroll[1]) ** 2 >= mindist: continue
                grass.append(
                    [
        ...
        for poly, color in self.road_poly:
            if (poly[0][0] - scroll[0]) ** 2 + (poly[0][1] - scroll[1]) ** 2 >= mindist: continue
...
for frame_skip in FRAME_SKIPS:
    gym.envs.register(
        id="CarRacingFS{}-v2".format(frame_skip),
